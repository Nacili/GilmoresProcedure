...
void prove(Signature::Sptr s, const Formula &f)
{
    Formula f1 = std::make_shared<Not>(f);
    std::cout << f1 << std::endl;
    gilmore(s, f1);
}

void gilmore(Signature::Sptr s, const Formula &f)
{

    Formula tf = f->removeUniversalQ(f->nnf()->prenex()->skolem(s));
    ...
    std::copy(vset.begin(), vset.end(), std::back_inserter(variables));
    ...
    for (iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
        std::vector< std::vector<Term> > substitutions =
            product(universe.getLevel(), nvars);
        ...
        for (LiteralList conjuncts : dnf) {
            ...
            for (auto literal : conjuncts) {
                if (BaseFormula::isOfType<Not>(literal)) {
                    Formula t = ((Not*)literal.get())->operand();
                ...
                } else if (BaseFormula::isOfType<Not>(literal)) {
